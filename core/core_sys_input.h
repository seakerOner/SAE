...
typedef enum PeripheralType_t {
  SAE_PERIPHERAL_T_UNKNOWN = 0,
  SAE_PERIPHERAL_T_KEYBOARD = 1,
  SAE_PERIPHERAL_T_MOUSE = 2,
  SAE_PERIPHERAL_T_GAMEPAD = 3,
} PeripheralType;
...
/*==================================================*/
/* API / TYPES IMPLEMENTATION                       */
/*==================================================*/

// temp
#define CORE_SYS_INPUT_IMPLEMENTATION

#if defined(CORE_SYS_INPUT_IMPLEMENTATION)
...
static int __sae_try_set_event_path(PeripheralDevice *peri) {
...
PeripheralDeviceList sae_get_available_peripherals_list() {
  ...
  // for (usize n_line = 0; n_line < buf_cap; n_line += 1) {
  while (*lines_buf != '\0') {
    ascii tmp_line[300];
